...
export default defineEventHandler(async (event) => {
  ...
  const db = useDb()
  const suratRes = await db.execute({ sql: `SELECT id, no_surat, file_drive_id, file_name, created_by FROM surat_masuk WHERE id=? AND deleted_at IS NULL`, args: [id] })
  ...
  const filesRes = await db.execute({ sql: `SELECT id, file_drive_id, file_name, mime_type FROM surat_files WHERE surat_masuk_id=? ORDER BY id ASC`, args: [id] })
  ...
  for (const r of rows) {
    ...
    try {
      ...
      const up = await uploadToDrive(`${surat.no_surat || id}_${newName}`, 'image/jpeg', buf, DROPBOX_FOLDERS.SM)
      ...
      if (surat.file_drive_id === r.file_drive_id || !surat.file_drive_id) {
        await db.execute({ sql: `UPDATE surat_masuk SET file_drive_id=?, file_name=? WHERE id=?`, args: [newId, newName, id] })
  ...
  await logActivity({ userId: auth.userId, action: 'REPAIR_SURAT_MASUK_IMAGE', entity: 'surat_masuk', entityId: id, detail: { repaired: repaired.length, skipped: skipped.length, failed: failed.length }, ip: getRequestIP(event, { xForwardedFor: true }) })
