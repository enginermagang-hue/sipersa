...
export default defineEventHandler(async (event) => {
  ...
  const db = useDb()
  const surat = await db.execute({ sql: `SELECT no_surat FROM surat_masuk WHERE id = ? AND deleted_at IS NULL`, args: [id] })
  if (surat.rows.length === 0) throw createError({ statusCode: 404, statusMessage: 'Surat tidak ditemukan' })
  const files = await db.execute({ sql: `SELECT file_drive_id, file_name FROM surat_files WHERE surat_masuk_id = ? ORDER BY id ASC`, args: [id] })
  let rows = files.rows as any[]
  if (!rows.length) {
    const legacy = await db.execute({ sql: `SELECT file_drive_id, file_name FROM surat_masuk WHERE id = ? AND file_drive_id IS NOT NULL`, args: [id] })
