...
export default defineEventHandler(async (event) => {
  ...
  if (auth.role !== 'pimpinan') throw createError({ statusCode: 403, statusMessage: 'Hanya pimpinan yang dapat menyetujui surat keluar' })
  ...
  if (surat.status !== 'menunggu_persetujuan') throw createError({ statusCode: 400, statusMessage: 'Surat tidak sedang menunggu persetujuan' })
  const { fields, file } = await readFormWithFile(event)
  const parsed = suratKeluarApprovalSchema.safeParse({ status: fields.status, catatan: fields.catatan || '' })
