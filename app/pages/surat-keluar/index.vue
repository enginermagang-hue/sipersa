...
const kpiCards = computed(() => {
  ...
  return [
    { label: 'Total Surat Keluar', value: stats.value?.total ?? 0, sub: 'Sejak Januari 2026', icon: 'i-lucide-send', bg: 'bg-violet-50 dark:bg-violet-950/50', color: 'text-violet-600 dark:text-violet-400' },
    { label: 'Draft & Ditolak', value: isPimpinan ? (stats.value?.ditolak ?? 0) : ((stats.value?.draft ?? 0) + (stats.value?.ditolak ?? 0)), sub: 'Perlu ditindaklanjuti', icon: 'i-lucide-file-text', bg: 'bg-slate-100 dark:bg-slate-800', color: 'text-slate-600 dark:text-slate-300' },
    { label: 'Menunggu Persetujuan', value: stats.value?.menunggu_persetujuan ?? 0, sub: 'Menunggu pimpinan', icon: 'i-lucide-pen-line', bg: 'bg-sky-50 dark:bg-sky-950/50', color: 'text-sky-600 dark:text-sky-400' },
    { label: 'Terkirim Bulan Ini', value: stats.value?.terkirim_bulan_ini ?? 0, sub: 'Bulan ini (WITA)', icon: 'i-lucide-check-circle', bg: 'bg-emerald-50 dark:bg-emerald-950/50', color: 'text-emerald-600 dark:text-emerald-400' }
  ]
})
...
function getThumb(r:any, size: 'small'|'medium'|'large' = 'small') {
  ...
  if (!isThumbable(name)) return null
              ...
              <td class="px-4 py-3 max-w-[320px]">
                <div class="flex items-center gap-2 min-w-0">
                  ...
                  <div class="min-w-0 max-w-[320px]">
                    <div class="font-medium flex items-center gap-1 truncate min-w-0" :title="r.no_surat"><span class="truncate">{{ r.no_surat }}</span><UBadge v-if="(r.file_count||0)>1" :label="`+${r.file_count}`" size="xs" variant="subtle" class="shrink-0" /></div>
      ...
      <div v-else-if="view === 'grid'" class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="r in data?.data || []" :key="r.id" class="rounded-xl border border-default p-4 transition-colors hover:bg-muted/30 cursor-pointer" @click="navigateTo(`/surat-keluar/${r.id}`)">
          ...
          <div class="flex items-start justify-between gap-2 min-w-0">
            <div class="font-medium text-sm leading-tight flex items-center gap-1 min-w-0 flex-1"><span class="truncate min-w-0" :title="r.no_surat">{{ r.no_surat }}</span><UBadge v-if="(r.file_count||0)>1" :label="`+${r.file_count}`" size="xs" variant="subtle" class="shrink-0" /></div>
