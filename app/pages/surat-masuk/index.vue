...
async function hapus(row: any) {
  const ok = await confirm({ title: 'Hapus Surat', message: `Hapus surat ${row.no_surat}?`, okLabel: 'Hapus' })
  if (!ok) return
...
<template>
  <div>
    ...
    <UModal v-model:open="addOpen" title="Tambah Surat Masuk">
      <template #body>
        <SuratForm v-if="addOpen" type="masuk" @close="onSaved" />
      </template>
    </UModal>

    <UModal v-model:open="editOpen" :title="`Edit Surat Masuk: ${editSurat?.no_surat || ''}`">
      <template #body>
        <SuratForm v-if="editOpen" type="masuk" :surat-id="editSurat?.id" :surat="editSurat" @close="onEditSaved" />
      </template>
    </UModal>
  </div>
</template>
